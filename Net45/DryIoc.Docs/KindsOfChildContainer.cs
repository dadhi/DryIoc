...
Instead DryIoc has number of APIs to address specific related scenarios, taking advantage of Container immutable state with very fast `O(1)` snapshots.

## With Open Scope

Method `OpenScope` produces a new container and [explained in more details here]
(ReuseAndScopes#markdown-header-incurrentscope).
...
class FacadeExample
{
    [Example] public void Facade_for_tests()
    {
        ...
        testFacade.Register<IService, TestService>(serviceKey: ContainerTools.FacadeKey);
