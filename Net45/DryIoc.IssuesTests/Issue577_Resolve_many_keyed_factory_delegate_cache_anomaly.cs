...
namespace DryIoc.IssuesTests
{
    ...
    public class Issue577_InconsistentResolutionAndCacheAnomaly
    {
        ...
        [Test] //RED
        public void ResolvingLazyDoesNotAddNewKeyedFactoryDelegatesToCache()
        {
            ...
            {
                //Resolve once to ensure all necessary factory delegates are created
                scope.Resolve<IStrategySelector<TransferImportItem>>();
            }

            //Resolve a second time to check if new factory delegates are added
            var cacheAfterFirstResolve = GetKeyedCache(container);
            ...
            var cacheAfterSecondResolve = GetKeyedCache(container);

            Assert.AreEqual(cacheAfterFirstResolve.Length, cacheAfterSecondResolve.Length);
            for (var i = 0; i < cacheAfterFirstResolve.Length; i++)
            {
                var first = cacheAfterFirstResolve[i];
                var secnd = cacheAfterSecondResolve[i];
                Assert.AreEqual(first.Key, secnd.Key);
                CollectionAssert.AreEquivalent(first.Value, secnd.Value);
            }
        }
